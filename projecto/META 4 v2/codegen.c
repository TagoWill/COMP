...
void generateCode(is_Nos* noactual, char* param, int tamanho)
{
	if(noactual != NULL){
		switch(noactual->queraioeisto){
			...
			case is_WRITELN:
				//printf("is_WRITELN\n");
			genWriteLn(noactual->nofilho, tamanho);
			generateCode(noactual->nonext, param, tamanho);
			break;
			case is_IFELSE:
				//printf("is_IFELSE\n");
		...
		switch(noactual->queraioeisto){
			...
			case is_MOD: ;
				...
				if(aux2 != NULL && aux3 != NULL && strcmp(aux2, "_integer_")==0 && strcmp(aux3, "_integer_")==0){
					switch(noactual->queraioeisto){
						case is_DIV:
							printf("%%%d udiv = i32 %%1, 1\n", tempname);
							tempname++;
							break;
						case is_MOD:
							printf("%%%d urem = i32 %%1, 1\n", tempname);
							tempname++;
							break;
					}
					return aux2;
			...
			case is_SUB: ;
				...
				if(aux2 != NULL && aux3 != NULL && strcmp(aux2, "_boolean_")!=0 && strcmp(aux3, "_boolean_")!=0
					...
					if(strcmp(aux2, "_integer_")==0 && strcmp(aux3, "_integer_")==0){
						switch(noactual->queraioeisto){
							case is_ADD:
								printf("%%%d add = i32 %%1, 1\n", tempname);
								tempname++;
								break;
							case is_SUB:
								printf("%%%d sub = i32 %%1, 1\n", tempname);
								tempname++;
								break;
							case is_MULT:
								printf("%%%d mul = i32 %%1, 1\n", tempname);
								tempname++;
								break;
						}
						return "_integer_";
						...
						switch(noactual->queraioeisto){
							case is_ADD:
								printf("%%%d add = double %%1, 1\n", tempname);
								tempname++;
								break;
							case is_SUB:
								printf("%%%d sub = double %%1, 1\n", tempname);
								tempname++;
								break;
							case is_MULT:
								printf("%%%d mul = double %%1, 1\n", tempname);
								tempname++;
								break;
						}
						return "_real_";
